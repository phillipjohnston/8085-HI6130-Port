...
#include "board_613x.h"
    ...
    #if(RT1_ena)
    extern const RT1d pRT1d;
    extern const RT1i pRT1i;
    #endif // (RT1_ena)
    
    #if(RT2_ena)
    extern const RT2d pRT2d;
    extern const RT2i pRT2i;
    #endif // (RT2_ena)
...
void initialize_613x_RT1(void) {
	...
	// ======================================================================================
		...
		for ( i = 0, k = (0x60000000 + j); i < 256; i++) {
		    *((volatile unsigned short *)(k)) = descr_table_RT1[i];
		    k += 2;
		}
		// in mode command half of table, just write Control Words, every 4th word
		for ( ; i < 512; i++) {
		    //  just write 4 host-maintained Control Word bits
		    *((volatile unsigned short *)(k)) = descr_table_RT1[i] & 0xF000; // desc word 1
		    k += 2;
		    *((volatile unsigned short *)(k)) = 0; // desc word 2 = 0
		    k += 2;
		    *((volatile unsigned short *)(k)) = 0; // desc word 3 = 0
		    k += 2;
		    *((volatile unsigned short *)(k)) = 0; // desc word 4 = 0
		    k += 2;
		    i += 3;
                }		
		...
		// write address k starts at table base address 
                ...
                for ( i = 0; i < 256; i++) {	
                    *((volatile unsigned short *)(k)) = illegal_table[i];
		    k = k + 2;
		}
...
void write_dummy_tx_data_RT1(void) {
	...
	unsigned short b_data[32] = {0xF001,0xF002,0xF003,0xF004,0xF005,0xF006,0xF007,0xF008,
		...
		for ( i = 0; i < 32; i++) {	
			*((volatile unsigned short *)(k)) = a_data[i];
			k = k + 2;
		}
		...
		// write the 32 data words...
		for ( i = 0; i < 32; i++) {	
			*((volatile unsigned short *)(k)) = b_data[i];
			k = k + 2;
		}
		...
		for ( i = 0; i < 32; i++) {	
			*((volatile unsigned short *)(k)) = 0xBADD;
			k = k + 2;
		}
		...
		// write the 32 data words...
		for ( i = 0; i < 32; i++) {	
			*((volatile unsigned short *)(k)) = 0xDEAD;
			k = k + 2;
		}
...
void initialize_613x_RT2(void) {
	...
	// ====================================================================================== 
		...
		// THE FIRST HALF OF THE TABLE FOR SUBADDRESS COMMANDS MUST BE FULLY INITIALIZED.  
			...
			// in subaddress command half of table, every word is written 
			for ( i = 0, k = (0x60000000 + j); i < 256; i++) {
				*((volatile unsigned short *)(k)) = descr_table_RT2[i];
				k += 2;
			}
			// in mode command half of table, just write Control Words, every 4th word 
			for ( ; i < 512; i++) {
				//  just write 4 host-maintained Control Word bits 
				*((volatile unsigned short *)(k)) = descr_table_RT2[i] & 0xF000; // desc word 1
				k += 2;
				*((volatile unsigned short *)(k)) = 0; // desc word 2 = 0
				k += 2;
				*((volatile unsigned short *)(k)) = 0; // desc word 3 = 0
