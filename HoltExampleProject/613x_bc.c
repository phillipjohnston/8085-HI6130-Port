        ...
        // copy BC Instruction List (above) to RAM...
            ...
            for ( i = 0; i < len; i += 2) {
                ...
                if (!((0x6996 >> j) & 1))  { 
                    ...
                    j = inst_list[i] + VP1;
                    *((volatile unsigned short *)(addr)) = j;
                }
                else {
                ...
                *((volatile unsigned short *)(addr)) = inst_list[i+1];
                addr += 2;
            }
...
void initialize_bc_msg_blocks(void) {
	...
	// ********************************************************************************
		...
		for ( i = 0; i < 8; i++) {
                        *((volatile unsigned short *)(addr)) = msg_block1[i]; // 0x3E00 - 0x3E07
			addr += 2;
		}

		for ( i = 0; i < 8; i++) {
			*((volatile unsigned short *)(addr)) = msg_block2[i]; // 0x3E08 - 0x3E0F
			addr += 2;
		}

		for ( i = 0; i < 8; i++) {
			*((volatile unsigned short *)(addr)) = msg_block3[i]; // 0x3E10 - 0x3E17
			addr += 2;
		}

		for ( i = 0; i < 8; i++) {
			*((volatile unsigned short *)(addr)) = msg_block4[i]; // 0x3E18 - 0x3E1F
			addr += 2;
		}


		for ( i = 0; i < 8; i++) {
			*((volatile unsigned short *)(addr)) = msg_block5[i]; // 0x3E20 - 0x3E27
			addr += 2;
		}

		for ( i = 0; i < 8; i++) {
			*((volatile unsigned short *)(addr)) = msg_block6[i]; // 0x3E28 - 0x3E2F
			addr += 2;
		}

		for ( i = 0; i < 8; i++) {
			*((volatile unsigned short *)(addr)) = msg_block7[i]; // 0x3E30 - 0x3E37
			addr += 2;
		}

		for ( i = 0; i < 8; i++) {
			*((volatile unsigned short *)(addr)) = msg_block8[i]; // 0x3E38 - 0x3E3F
			addr += 2;
		}
		...
		for ( i = 0; i < 16; i++) {
			*((volatile unsigned short *)(addr)) = rtrt_msg_block1[i]; // 0x3E40 - 0x3E4F
			addr += 2;
		}

		for ( i = 0; i < 16; i++) {
			*((volatile unsigned short *)(addr)) = rtrt_msg_block2[i]; // 0x3E50 - 0x3E5F
			addr += 2;
		}
		...
		for ( i = 0; i < 32; i++) {	
			*((volatile unsigned short *)(addr)) = data[i];
			addr += 2;
		}
		...
		for ( i = 0; i < 16; i++) {	
			...
			*((volatile unsigned short *)(addr)) = data[i+15];
			addr += 2;
		}
...
void bc_disable(void) {
	...
	unsigned short j = (pH6130->MASTER_CONFIG_REG) & ~BCENA;
...
void bc_start(void) {
	...
	if(PIO_Get(&pinBCENA)) {
            ...
            // read-modify-write Master Config register
            unsigned short j = (pH6130->MASTER_CONFIG_REG) | BCENA | BCSTRT ;
	    pH6130->MASTER_CONFIG_REG = j;
