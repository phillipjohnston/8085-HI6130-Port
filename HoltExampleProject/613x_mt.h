 ...
 * 	        definitions used by functions in 613x_mt.c file. 
 *
...
typedef volatile unsigned short (*DSTK)[DSTACKSIZE] ;
...
typedef struct A_List {
  volatile unsigned short cmdStkStart;  // command stack start address
  volatile unsigned short cmdStkNext;   // command stack next messsage block address
  volatile unsigned short cmdStkEnd;    // command stack end address
  volatile unsigned short cmdStkInt;    // optional cmd stack fill warning interrupt address
  volatile unsigned short dataStkStart; // data stack start address
  volatile unsigned short dataStkNext;  // data stack next messsage data address
  volatile unsigned short dataStkEnd;   // data stack end address
  volatile unsigned short dataStkInt;   // optional data stack fill warning interrupt address
} addList, *AddrList;
...
typedef struct SMsgBlock {
  volatile unsigned short ttagLow;
  volatile unsigned short ttagMid;
  volatile unsigned short ttagHigh;
  volatile unsigned short blockStatus;
  struct respTime gaptime;
  volatile unsigned short msgBytes;
  volatile unsigned short dataPtr;
  volatile unsigned short cmdWord;
...
typedef struct SMsgBlock {
  volatile unsigned short blockStatus;
  volatile unsigned short timeTag;
  volatile unsigned short dataPtr;
  volatile unsigned short cmdWord;
} SMT_cmdStack[SMT_BLKS], (* ScSTK)[SMT_BLKS];
...
typedef struct A_List {
  volatile unsigned short stkStartAddr;    // stack start address
  volatile unsigned short currPacketAddr;  // start address for current packet
  volatile unsigned short stkEndAddr;      // stack end address
  volatile unsigned short stkIntAddr;      // stack address for optional level-attained interrupt 
  volatile unsigned short lastBSWAddr;     // address of last-written block status word
  volatile unsigned short dummy[2];        // 2 unused words
  volatile unsigned short pktOvrWordsInt;  // number of words to packet overrun, optional warning interrupt 
} addList, *AddrList;
...
typedef struct PktHdrCSD {
  volatile unsigned short pktSync;
  volatile unsigned short ChannID;
  volatile unsigned short pktLenLo;
  volatile unsigned short pktLenHi;
  volatile unsigned short dataLenLo;
  volatile unsigned short dataLenHi;
  struct hWord6 word6;
  struct hWord7 word7;
  volatile unsigned short relTimeLow;
  volatile unsigned short relTimeMid;
  volatile unsigned short relTimeHigh;
  volatile unsigned short hdrChecksum;
  volatile unsigned short numMsgsLo;
...
typedef struct IntraPktMsgBlk {
  ...
  volatile unsigned short msgTime0;
  volatile unsigned short msgTime1;
  volatile unsigned short msgTime2;
  volatile unsigned short msgTime3;
  // Intra-Packet Data Header (3 words, 4 fields)
  volatile unsigned short blkStatWd;
  struct respTime gap;
  volatile unsigned short msgLength;
} *IPMB;
